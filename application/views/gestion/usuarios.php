	...
	<div id="cargar_carteras" class="capa" style="display:none">
		...
		<form action="<?= base_url() ?>gestion/carga_csv_asignacion/" method="post" enctype="multipart/form-data" target="iframe_csv">
			<table class="table">
				...
				<tr>
					<td>Selecciona el ejecutivo:<br>
						<select name="ejecutivo" id="ejecutivo">
							<option value="0">--</option>
							<?php foreach($usuarios as $us): if($us['visible']==1){ ?>
								<option value="<?= $us['ID'] ?>"><?= $us['NOM_EJECUTIVO'] ?></option>
							<?php } endforeach; ?>
						</select>
					</td>
				</tr>
		...
		<iframe width="100%" height="450" src="<?= base_url() ?>gestion/carga_csv_asignacion/" frameborder="0" id="iframe_csv" name="iframe_csv"></iframe>
	</div>
</div>
</div>
