<?php if ( ! defined('BASEPATH')) exit('No se permite el acceso directo al script');
	...
	class Mod_clientes extends CI_Model
	{
		...
		{
			$query = $this->db->query("SELECT CC.COMUNA,count(CCA.idrbd) as colegios,sum(CC.ALUMNOS_SEP) as alumnos_sep, count(CCG.rbd) as gestiones_colegios
										FROM core_clientes_sep CC JOIN core_clientes_asignaciones CCA
										ON CC.RBD = CCA.idrbd LEFT JOIN core_cliente_gestion CCG ON CCG.RBD = CCA.idrbd
										WHERE CCA.idusuario = '".$id['id']."'
										GROUP BY CC.COMUNA");
			return $query->result_array();
		}
